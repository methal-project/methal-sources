...
"""
Process character info for Hofnarr Heidideldum and add it to the TEI file.
"""
...
def add_character_infos_to_tei(df, teifn, teifn_with_characters,
    ...
    # add dedication ==========================================================
    de = ut.get_dedication_by_id(ocf.dedication_fn, cf.play_id)
    af.add_dedication_to_front(front, de, page_after_add=2)
    ...
    for sp in ttree.xpath("//tei:sp", namespaces=NSMAP_READ):
        ...
        try:
            ...
            try:
                ...
                sp.attrib["who"] = sp2id[spkr.strip()+":"]
