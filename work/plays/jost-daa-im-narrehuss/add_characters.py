...
"""
Process character info for E Daa im Narrehüs and add it to the TEI file.
"""
...
def add_character_infos_to_tei(df, teifn, teifn_with_characters,
    ...
    for sp in ttree.xpath("//tei:sp", namespaces=NSMAP_READ):
        ...
        # added for Narrehüss but I meant for speakers to be copied to sp2id.tsv
        # with same capitalization as in the text of the play
        #TODO: see if want a normalization for speaker names here, in general
        spkr = spkr[0] + spkr[1:].lower() if spkr[0] not in ("1", "2", "3") else spkr[0:4] + spkr[4:].lower()
        try:
